...
$app->post ('/register/tst',function(Request $request, Response $response){
    ...
    /*console.log($id);*/
    $sql = "SELECT * FROM Register WHERE RegNumber = $id";
    ...
    try{
        ...
        $ret = $pharmacists[0][password];
        if($ret){
            if ($password == $ret){
                echo $pharmacists[0][RegNumber];
...
$app->post ('/register/getconsultations',function(Request $request, Response $response){
    $id = $request->getParam('value');
    /*console.log($id);*/
    $sql = "SELECT * FROM consultations Where patientRefID = $id";
    /*$sq = "INSERT INTO login_tokens (user_id, token) VALUES (:user, :token)";*/
    try{
        $db = new db();
        $db = $db->connect();
...
$app->post ('/register/getcustomer',function(Request $request, Response $response){
    $id = $request->getParam('value');
    /*console.log($id);*/
    $sql = "SELECT * FROM PatientDetails Where patientID = $id";
    /*$sq = "INSERT INTO login_tokens (user_id, token) VALUES (:user, :token)";*/
    try{
        $db = new db();
        $db = $db->connect();
...
$app->get('/register/add', function(Request $request, Response $response){
    $firstname = $request->getParam('firstname');
    $lastname = $request->getParam('lastname');
    $RegNumber = $request->getParam('RegNumber');
    ...
    $sql2 = "INSERT INTO `Register` (`ROLE`, `RegNumber`, `LastName`, `FirstName`, `Email`, `password`) VALUES ('2', '987', 'name2', 'name1', 'email@email.com', 'pass2');";
    try{
        // Get DB Object
        $db = new db();
        // Connect
        ...
        delete_token($RegNumber);
    } catch(PDOException $e){
        echo '{"sign up error": {"text": '.$e->getMessage().'}';
    }
